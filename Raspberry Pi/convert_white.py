...
work_dir = os.path.join('e:', os.sep, 'Files', 'Code', 'Python', 'Projects', 'Lego_Sorter', 'misc')
input_dir = os.path.join(work_dir, 'photo.jpg')
output_dir = os.path.join(work_dir, 'output.jpg')

bgr_input = cv2.imread(input_dir)
hsv_input = cv2.cvtColor(bgr_input, cv2.COLOR_BGR2HSV)

mask = cv2.inRange(hsv_input, np.array([0, 0, 0]), np.array([120, 255, 255]))
